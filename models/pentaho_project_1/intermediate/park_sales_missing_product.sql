...
-- A row first appears when its product is missing. On later runs the same row is
-- retained and marked resolved when that product becomes available.
...
exception_candidates as (
    ...
    where current_sales.is_missing_product
...
select
    ...
    discount_amount,
    source_updated_at,
    'Missing product_id in dim.dim_product_source' as rejection_reason,
    coalesce(previous_parked_at, current_timestamp()) as parked_at,
    not is_missing_product as is_resolved,
    case
        when not is_missing_product
